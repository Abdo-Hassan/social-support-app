...
const getApiKey = (): string | null => {
  ...
  // For this demo, we'll use a placeholder that users need to set
  return process.env.OPENAI_API_KEY || null;
};

// System prompts for different fields
const getSystemPrompt = (field: AIAssistanceRequest['field'], context: AIAssistanceRequest['context']): string => {
...
- Monthly Income: ${context.monthlyIncome ? `$${context.monthlyIncome}` : 'Not provided'}
...
export const generateAIAssistance = async (request: AIAssistanceRequest): Promise<AIAssistanceResponse> => {
    ...
    const response = await axios.post(
      OPENAI_API_URL,
      {
        ...
        messages: [
          ...
          {
            role: 'user',
            content: `Please help me write a ${request.field.replace(/([A-Z])/g, ' $1').toLowerCase()} description for my social support application.`
          }
        ],
        max_tokens: 500,
        temperature: 0.7,
...
export const generateMockAIAssistance = async (request: AIAssistanceRequest): Promise<AIAssistanceResponse> => {
  // Simulate API delay
