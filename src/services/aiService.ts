...
- الدخل الشهري: ${context.monthlyIncome ? `$${context.monthlyIncome}` : 'غير محدد'}
...
export const generateAIAssistance = async (request: AIAssistanceRequest): Promise<AIAssistanceResponse> => {
  ...
  try {
    ...
    const userPrompt = isArabic 
      ...
      : `Please help me write a ${request.field.replace(/([A-Z])/g, ' $1').toLowerCase()} description for my social support application.`;
